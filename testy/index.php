...
<?php
	...
	$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT test_id, test_name, subject_name, test_start, test_stop, test_rep, IF(ISNULL(solutions), 0, solutions) AS solutions, IF(ISNULL(contin), 0, contin) AS contin FROM SC_tests LEFT JOIN SC_subjects ON test_subj=subjectsID INNER JOIN SC_class_perm ON test_id=cp_test INNER JOIN SC_users ON cp_class=class LEFT JOIN (SELECT score_user, score_test, COUNT(score_id) AS solutions, IF(MIN(score_stop)='0000-00-00 00:00:00', 1, 0) AS contin FROM SC_score GROUP BY score_user, score_test) AS info ON usersID=score_user AND test_id=score_test WHERE usersID=".$userID." AND test_stop>'".$today."' UNION SELECT test_id, test_name, subject_name, test_start, test_stop, test_rep, IF(ISNULL(solutions), 0, solutions) AS solutions, IF(ISNULL(contin), 0, contin) AS contin FROM SC_tests LEFT JOIN SC_subjects ON test_subj=subjectsID INNER JOIN SC_school_perm ON test_id=sp_test INNER JOIN SC_users ON sp_school=school LEFT JOIN (SELECT score_user, score_test, COUNT(score_id) AS solutions, IF(MIN(score_stop)='0000-00-00 00:00:00', 1, 0) AS contin FROM SC_score GROUP BY score_user, score_test) AS info ON usersID=score_user AND test_id=score_test WHERE usersID=".$userID." AND test_stop>'".$today."' UNION SELECT test_id, test_name, subject_name, test_start, test_stop, test_rep, IF(ISNULL(solutions), 0, solutions) AS solutions, IF(ISNULL(contin), 0, contin) AS contin FROM SC_tests LEFT JOIN SC_subjects ON test_subj=subjectsID INNER JOIN SC_group_perm ON test_id=grp_test INNER JOIN SC_gr_user ON grp_group=group_id LEFT JOIN (SELECT score_user, score_test, COUNT(score_id) AS solutions, IF(MIN(score_stop)='0000-00-00 00:00:00', 1, 0) AS contin FROM SC_score GROUP BY score_user, score_test) AS info ON gruser=score_user AND test_id=score_test WHERE gruser=".$userID." AND test_stop>'".$today."' UNION SELECT test_id, test_name, subject_name, test_start, test_stop, test_rep, IF(ISNULL(solutions), 0, solutions) AS solutions, IF(ISNULL(contin), 0, contin) AS contin FROM SC_tests LEFT JOIN SC_subjects ON test_subj=subjectsID INNER JOIN SC_users_perm ON test_id=up_test LEFT JOIN (SELECT score_user, score_test, COUNT(score_id) AS solutions, IF(MIN(score_stop)='0000-00-00 00:00:00', 1, 0) AS contin FROM SC_score GROUP BY score_user, score_test) AS info ON up_user=score_user AND test_id=score_test WHERE up_user=".$userID." AND test_stop>'".$today."' UNION SELECT test_id, test_name, subject_name, test_start, test_stop, test_rep, IF(ISNULL(solutions), 0, solutions) AS solutions, IF(ISNULL(contin), 0, contin) AS contin FROM SC_tests LEFT JOIN SC_subjects ON test_subj=subjectsID INNER JOIN SC_glob_perm ON test_id=gp_test LEFT JOIN (SELECT score_test, COUNT(score_id) AS solutions, IF(MIN(score_stop)='0000-00-00 00:00:00', 1, 0) AS contin FROM SC_score WHERE score_user=".$userID." GROUP BY score_test) AS info ON test_id=score_test WHERE test_stop>'".$today."') AS data ORDER BY test_stop ASC LIMIT ".(15*($page-1)).", 15;";
	$result = $conn->query($sql);
	
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			echo '<tr><td class="start-table-name"><a href="'.$httplocal.'/info/'.$row["test_id"].'/">'.$row["test_name"].'</a></td><td class="start-table-date">'.$row["subject_name"].'</td><td class="start-table-date">'.date("d-m-Y H:i", strtotime($row["test_start"])).'</td><td class="start-table-date">'.date("d-m-Y H:i", strtotime($row["test_stop"])).'</td><td class="start-table-action"><a href="'.$httplocal.'/info/'.$row["test_id"].'/" class="start-table-button">Info</a>';
			if(strtotime($row["test_start"]) < strtotime($today)+600){
				if(($row["test_rep"] == 0) || ($row["test_rep"] > $row["solutions"])){
					if($row["contin"] == 0){
						echo '<a href="'.$httplocal.'/rozwiaz/'.$row["test_id"].'/" class="start-table-button" title="Rozwiąż nowy test">Rozpocznij</a>';
					} else {
						echo '<a href="'.$httplocal.'/rozwiaz/'.$row["test_id"].'/" class="start-table-button" title="Dokończ rozpoczęty test">Kontynuuj</a>';
					}
